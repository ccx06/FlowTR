...
class QueryDecompose(Action):
    """[Currently only supports **single table**] Decompose and expand query questions. """
    name: str = "QueryDecompose"
    ...
    async def run(self, inputs: str):
        ...
        if 'entity_info' in inputs:
            entity_info = inputs['entity_info']
            prompt = prompt + '\n\n' + f'Pay attention to the correspondence between entities in user queries (`key` field) and table content (`value` field): \n{entity_info}\n\nYou can refer to the above content to generate more concrete steps.'
